...
def model_name_to_output(model_path, conf):
    ...
    return f"merged_classify__{name}_conf_{conf:.2f}.txt"
...
# ----------------------------------------
def split_images(dataset_path):
    base_root = dataset_path
...
def run_prediction(PREDICT_MODEL, settings):
    ...
    print("\nPredicting images1...")
    predict_folder(model, settings["dataset_path"], output_name, settings)
