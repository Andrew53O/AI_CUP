...
results = model.train(
   ...
   epochs=150,          # 150 is sufficient if convergence is clean
    batch=8,
    imgsz=640,
    device=0,
    patience=20,
    optimizer='AdamW',
    dropout=0.05,        # Reduced dropout to let model learn faster
    
    # --- PRECISION MEDICAL AUGMENTATION ---
    # The goal is robustness without destroying anatomy
    degrees=5,           # Small rotation (±5°)
    flipud=0.0,          # Vertical flip is BAD for hearts
    fliplr=0.5,          # Horizontal flip is OK
    scale=0.05,          # Mild zoom (±25%) - keeps valve visible
    ...
    # Refinement Strategy
    cos_lr=True,
    # close_mosaic=50,     # Train on REAL images for the last 50 epochs (Crucial!)
)