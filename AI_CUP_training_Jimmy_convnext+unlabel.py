...
def move_patients(start, end, split):
    for i in range(start, end + 1):
        ...
        for j in range(0, len(unlabeled), step):
            img_path = os.path.join(img_dir, unlabeled[j] + ".png") # get the unlabeled image path
            shutil.copy2(img_path, f"./datasets/{split}/images/") # copy to dataset train 
...
model = YOLO('yolo12m.yaml') #初次訓練使用YOLO官方的預訓練模型，如要使用自己的模型訓練可以將'yolo12n.pt'替換掉
